...
#
# Scope so far: CLI, daemon, group, udev rule, config, TLP drop-in and the
# boot service. The GNOME extension (M4) and the panel (M5) come later.
set -euo pipefail
...
say "1/8 Compilando em release (sem root)"
...
say "3/8 Binários em $BIN"
run_sudo install -m 0755 target/release/alienfan target/release/alienfand "$BIN/"

say "4/8 Regra udev (escrita do grupo no perfil e no boost)"
...
say "6/8 Drop-in do TLP (o TLP deixa de trocar o perfil)"
...
say "7/8 Serviço de boot (aplica o padrão antes do login)"
...
say "8/8 Daemon da sessão (alienfand)"
